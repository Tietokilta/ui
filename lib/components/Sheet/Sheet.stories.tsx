...
const ListSubLink = ({ className, ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
  <Button asChild variant="link" className={cn("w-full border-b-0", className)}>
    <a {...props} />
  </Button>
);

const BigListSublink = ({ className, ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
  <Button asChild variant="outlineLink" className={cn("w-full justify-start gap-2", className)}>
    <a {...props} />
  </Button>
);

const ListLink = ({ className, ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
              ...
              {"href" in linkOrGroup ? (
                ...
                <Collapsible className="space-y-3">
                  <CollapsibleTrigger className="group flex items-center gap-2">
                    ...
                    <span className="sr-only block group-data-[state=open]:hidden">Open</span>
                    <ChevronUpIcon className="hidden h-6 w-6 group-data-[state=open]:block" />
                    <span className="sr-only hidden group-data-[state=open]:block">Close</span>
                  </CollapsibleTrigger>
                    ...
                    {linkOrGroup.links.map((linkCategorySublist) => (
                      <ul key={linkCategorySublist.title}>
                        ...
                        {linkCategorySublist.links.map((link) => (
                          <li key={link.title}>
                            {"icon" in link ? (
                              <BigListSublink href="#" className="my-3 flex items-center gap-2">
                                {link.icon}
          ...
          <li>
            <ul className="flex items-center justify-center gap-6">
              {socialLinks.map((link) => (
                <li key={link.title}>
                  <a className="-m-2 block rounded-full p-2 hover:bg-gray-300" href="#">
                    {link.icon}
